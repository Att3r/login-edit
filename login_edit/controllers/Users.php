    ...
    class Users {
        ...
        public function register(){
            ...
            empty($data['usersPwd']) || empty($data['pwdRepeat'])){
                flash("register", "Palun täida kõik väljad");
                redirect("../signup.php");
            }
            ...
            if(!filter_var($data['usersEmail'], FILTER_VALIDATE_EMAIL)){
                flash("register", "Vale email");
                redirect("../signup.php");
            }

            if(strlen($data['usersPwd']) < 6){
                flash("register", "Vale parool");
                redirect("../signup.php");
            } else if($data['usersPwd'] !== $data['pwdRepeat']){
                flash("register", "Paroolid ei ole samad");
                redirect("../signup.php");
            }

            //User with the same email or password already exists
            if($this->userModel->findUserByEmailOrUsername($data['usersEmail'], $data['usersName'])){
                flash("register", "Kasutajanimi või email on juba kasutusel");
                redirect("../signup.php");
            }
